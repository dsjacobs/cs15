...
// Constructor that creates a linked list from a given
// array and size. The size is set to the given
...
void CharLinkedList::removeAt(int index)
{
    ...
    if (isEmpty() or index > (currSize - 1) or index < 0) {
        std::string error = errorBuilder(index, currSize, ')');
        throw(std::range_error(error));
    }
...
//private nodeAt helper that finds the node at the index
CharLinkedList::Node
...
// private function that takes an index and a desired closing character(either 
// ')' or ']' ) and builds a standard error
...
// private function that takes an index and a string and supplies the standard 
// part of 'toString' and 'toReverseString' to build a full string
...
// default node without a value has null next and prev pointers 
CharLinkedList::Node::Node() {
    next = nullptr;
    prev = nullptr;
};

// default node with given value and null next and prev pointers 
...
// returns address of new node with no value
...
// returns address of new node with value
...
void CharLinkedList::deep_copy(const CharLinkedList &other) {
